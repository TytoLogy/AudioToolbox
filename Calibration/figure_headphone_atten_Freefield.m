function [atten_val, spl_val] = figure_headphone_atten_Freefield(spl_val, rms_val, caldata, CarrierFREQ, varargin)
%---------------------------------------------------------------------
...
%       CarrierFREQ Vector of 1 or 2 values indicating tone or hi and lo 
%                   cutoff freqs for noise
% 
% 		Optional:
...
loweridx = find(caldata.freq==CarrierFREQ(1));
upperidx = find(caldata.freq==CarrierFREQ(end));
freqrangeidx = [loweridx:upperidx];

% MAYBE THIS INSTEAD
[a,loweridx]=min(abs(round(caldata.freq - CarrierFREQ(1))));
[a,upperidx]=min(abs(round(caldata.freq - CarrierFREQ(end))));
freqrangeidx = [loweridx:upperidx];

if spl_val(1) == 0
	atten_val(1) = MAXATTEN;
else
	atten_val(1) = min(caldata.mag(1,freqrangeidx)) + db(rms_val(1)) - spl_val(1);
    prevatten(1) = caldata.mindbspl(1)              + db(rms_val(1)) - spl_val(1);
end
...
if spl_val(2) == 0
	...
	atten_val(2) = min(caldata.mag(2,freqrangeidx)) + db(rms_val(2)) - spl_val(2);
    prevatten(2) = caldata.mindbspl(2)              + db(rms_val(2)) - spl_val(2);
end
